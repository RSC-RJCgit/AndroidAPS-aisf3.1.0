...
class DetermineBasalAutoISF @Inject constructor(
    ...
    */
    ...
    fun determine_basal(
        ...
        if (autoIsfMode) {
            consoleError.add("--------------- -------------------")
            consoleError.add("start AutoISF ${profile.autoISF_version}  RSNp082")
            consoleError.add("----------------------------------")
        ...
        rT.reason.append(
            "RSNp082 COB: ${round(meal_data.mealCOB, 1).withoutZeros()}, Dev: ${convert_bg(deviation.toDouble())}, BGI: ${convert_bg(bgi)}, ISF: ${convert_bg(sens)}, CR: ${
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        if (iob_data.iob > max_iob) {
            ...
            if (microBolusAllowed && enableSMB && bg > threshold) {
                ...
                consoleError.add("offsetSoZeroSMB $offsetSoZeroSMB")
                if ((( nowHour  >= 22 ) || ( nowHour  >= 6 && nowHour <=8 )) && bg < 7.5 * 18 && Delta <0.8 * 18  && SDelta <0.6 * 18 ) {// SemiTwilight and SMB over 0.5
