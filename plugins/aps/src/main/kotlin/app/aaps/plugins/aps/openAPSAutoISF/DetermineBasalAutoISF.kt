...
class DetermineBasalAutoISF @Inject constructor(
    ...
    */
    ...
    fun determine_basal(
        ...
        if (autoIsfMode) {
            consoleError.add("--------------- -------------------")
            consoleError.add("start AutoISF ${profile.autoISF_version}Nrsn100")
            consoleError.add("----------------------------------")
        ...
        rT.reason.append(
            "Nrsn100 COB: ${round(meal_data.mealCOB, 1).withoutZeros()}, Dev: ${convert_bg(deviation.toDouble())}, BGI: ${convert_bg(bgi)}, ISF: ${convert_bg(sens)}, CR: ${
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        if (iob_data.iob > max_iob) {
            ...
            if (microBolusAllowed && enableSMB && bg > threshold) {
                ...
                if (lastBolusAge > SMBInterval - 16.0) {   // changed from 6s tolerance
                    if (microBolus > 0) {
                        rT.units = microBolus
